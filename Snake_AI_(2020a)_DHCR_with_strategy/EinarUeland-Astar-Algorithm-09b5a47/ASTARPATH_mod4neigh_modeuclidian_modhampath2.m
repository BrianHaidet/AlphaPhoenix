function OptimalPath=ASTARPATH(StartX,StartY,MAP,GoalRegister,Connecting_Distance,hampathmap)
%Version 1.0
...
for k=1:size(GoalRegister,1)
    for j=1:size(GoalRegister,2)
        if MAP(k,j)==0
            ...
            Dist1=(min((sum(abs(Mat),2))));%
            %Distance=(min(sqrt(sum(abs(Mat).^2,2))));
            Dist2=max(hampathmap(k,j)-(Width*(Height-1)),0);
...
Hn=Hn+1;
Hn(hampathmap==hampathmap(StartY,StartX)-1)=0;
